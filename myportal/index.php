...
<head>
  ...
  <title>AttendanceHub - My Portal</title>
...
$mn_timecard=$conn->prepare("SELECT b.attendanceid,a.firstname,a.lastname,DATE_FORMAT(b.date_in,'%b %d, %Y') as date_in,TIME_FORMAT(b.time_in,'%h:%i %p') as time_in ,DATE_FORMAT(b.date_out,'%b %d, %Y') as date_out,TIME_FORMAT(b.time_out,'%h:%i %p') AS time_out,b.tasks,CONCAT(HOUR(TIMEDIFF(CONCAT(date_in,' ',time_in),CONCAT(date_out,' ',time_out))),' hours, ',MINUTE(TIMEDIFF(CONCAT(date_in,' ',time_in),CONCAT(date_out,' ',time_out))),' minutes') AS timerendered,a.level,TIME_FORMAT(d.shift_in,'%h:%i %p') AS shift_in,TIME_FORMAT(d.shift_out,'%h:%i %p') AS shift_out,b.NSD FROM users a INNER JOIN attendance b ON a.userid = b.userid INNER JOIN schedule c ON a.userid = c.userid
INNER JOIN shift d ON c.shiftid = d.shiftid WHERE b.date_in BETWEEN c.scheduled_date_start AND c.scheduled_date_end AND a.userid = :userid
GROUP BY b.date_in,b.time_in ORDER BY b.date_in DESC,b.time_in DESC");

$mn_timecard->execute(array(':userid'=>$userdata));
...
<body id="page-top">
  ...
  <div id="wrapper">
    ...
    <!-- Sidebar -->
    <?php include "../includes/L1_sidebar.html"; ?>
    <!-- End of Sidebar --> 
...
?>
    ...
    <tr style="font-size: 11px">
      ...
      <td><?php echo $row_timecard['attendanceid']; ?></td>
     ...
     <?php echo $row_timecard['time_out']; ?></td>
      ...
      <td title="<?php echo 'NSD: '.$row_timecard['NSD']; ?>"><?php if(!empty($row_timecard['time_out'])){echo $row_timecard['timerendered'];} else{echo "PENDING";} ?></td>
